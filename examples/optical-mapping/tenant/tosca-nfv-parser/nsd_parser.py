        ...
        for fp_name in fps.keys():
            ...
            for path_node in fp:
                if "CP" in path_node:
                    ...
                    node['domain'] = "dc1" if path_node == "CP01" else "dc2"
                    nodes.append(node)
                    ...
                    node['vdu-name'] = vnf_node.get_vdu().get_vdu_name()
                    nodes.append(node)
    ...
    def _write_to_file(self, fp_acl_dict, fp_nodes_tuples):
        ...
        for fp_nodes_tuple in fp_nodes_tuples:
            ...
            for node in forwarding_nodes:
                ...
                if 'vdu-name' in node.keys():
                    ET.SubElement(node_tag, "vdu-name").text = node['vdu-name']
